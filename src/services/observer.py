...
class DownloadHandler(FileSystemEventHandler):
    ...
    A handler is bound to one watch location; ``rules`` scopes that
    location's rule set. ``None`` (default) means the global config rules.
    """

    def __init__(self, rules=None):
        self.rules = rules_agent if rules is None else RulesAgent(rules)
    ...
    def _process_file(self, file_path: Path):
        ...
        matches = self.rules.evaluate(file_path)
...
class ObserverService:
    ...
    Step 7.2: one location is now a list — ``watch_locations`` of
    ``{"path": ..., "rules": [...]?}``. Each location gets its own
    DownloadHandler with its own scoped rules; legacy ``watch_directory``
    still seeds the list when ``watch_locations`` is empty.
    """
    ...
    def _resolve_locations(self) -> List[Dict[str, Any]]:
        """[{path, rules?}] from watch_locations, or legacy watch_directory."""
    ...
    def start(self):
        ...
        if not locations:
            logger.error("No watch locations configured.")
            return

        existing = [(Path(loc["path"]), loc) for loc in locations if Path(loc["path"]).exists()]
        if not existing:
            logger.error("None of the configured watch locations exist.")
            return
        ...
        for path, loc in existing:
            ...
            handler = DownloadHandler(rules=loc.get("rules"))
    ...
    def sync_existing_files(self):
        """Iterates through existing files in every watch location and organizes them."""
        for loc in self._resolve_locations():
            path = Path(loc["path"])
            if not path.exists():
                continue
            ...
            logger.info(f"Initial sync complete for: {path}")

    def restart_if_needed(self, new_config: Dict[str, Any]):
        """Restarts the observer if monitoring was toggled, or paths/rules changed."""
