...
- **Fixed (`2951940`):** `logger` imported in `classifier.py`.
...
- **Fixed (`2951940`):** `test_nlp_db.py` uses `get_nlp_service()`.
...
- **Fixed (`2951940`):** imports platform-guarded; startup registration no-ops off-Windows.
...
- **Fixed (`2951940`):** readiness retry replaces the fixed sleep; README claims corrected (see below).

### C6. `requirements.txt` cannot install on Linux — CI dies before tests run
...
- **Fix:** environment-markers — `winshell; sys_platform == "win32"` and `pypiwin32; sys_platform == "win32"` (pip's native conditional syntax). Linux installs skip both; Windows still gets them.
...
- **Fixed:** all DB operations log structured ERROR entries on failure; `upsert_file` returns success boolean. Verified by failure-injection test.
...
- **Fixed:** WAL mode + single persistent connection guarded by a lock. Verified by concurrent-thread stress test.
...
- **Fixed:** append-only versioned journal schema + write-before-action journaling in `organizer.move_file` ([ADR-013](decisions/ADR-013-append-only-transaction-journal.md)); safe reverse replay `Organizer.undo_last()` + provenance `journal_provenance()` ([ADR-016](decisions/ADR-016-safe-journal-backed-undo.md), [undo tests](../tests/test_undo.py)).
...
- **Fixed:** canonical homes via `platformdirs` (config → `user_config_dir`, logs → `user_log_dir`, DB → `user_data_dir`) with a one-time migration of the legacy `config/config.json` ([ADR-014](decisions/ADR-014-cross-platform-platformdirs.md)).
...
**Net:** the old app wasn't uniquely bad — it was *behind* an ecosystem that had already solved each of these. The upgrade adopts the industry norms: progressive hashing, content-first classification, dry-run everywhere, undo journal, and surface-don't-punish semantics.
